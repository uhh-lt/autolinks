...
BEGIN
  ...
  START TRANSACTION READ WRITE;
    if name_ is NOT NULL then
      select did into did_ from documents where name = name_ and uid = uid_ limit 1 LOCK IN SHARE MODE;
      if did_ = 0 then
...
BEGIN
  ...
  select r1.rid into rid_ from (select rid from resources where uid = uid_ LOCK IN SHARE MODE) r1, (select rid from stringResources where surfaceform = surfaceform_ LOCK IN SHARE MODE) r2 where r1.rid=r2.rid;
  if rid_ > 0 then
    return rid_;
  end if;
...
BEGIN
  declare rid_ int unsigned default 0;
  select r1.rid into rid_ from (select rid from resources where uid = uid_ LOCK IN SHARE MODE) r1, (select rid from tripleResources where subj = subj_ and pred = pred_ and obj = obj_ LOCK IN SHARE MODE) r2 where r1.rid=r2.rid;
  if rid_ > 0 then
    return rid_;
  end if;
...
BEGIN
  ...
  select r1.rid into rid_ from (select rid from resources where uid = uid_ LOCK IN SHARE MODE) r1, (select rid from listResources where listdescriptor = listdescriptor_ LOCK IN SHARE MODE) r2 where r1.rid=r2.rid;
  if rid_ > 0 then
    return rid_;
  end if;
